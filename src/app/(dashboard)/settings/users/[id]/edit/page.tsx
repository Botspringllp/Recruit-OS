...
import { updateUserAction, getUserByIdAction, getUsersAction } from '@/app/actions/users';
...
export default function EditUserPage({ params }: EditUserPageProps) {
  ...
  useEffect(() => {
    Promise.all([
      ...
      if (userRes.success && userRes.data) {
        ...
        const existingPerms = (u.permissions || []).map((p: any) => `${p.resource}.${p.action}`);
        setSelectedPermissions(existingPerms);
      } else {
        setErrorMessage(userRes.error || 'Failed to load user record');
      }

      if (usersRes.success && usersRes.data?.users) {
        setAgencyUsers(usersRes.data.users.filter((usr: any) => usr.id !== id));
      }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      ...
      if (res.success) {
        window.location.href = `/settings/users/${id}`;
      } else {
  ...
  if (isLoading) {
    return (
      <div className="bg-white min-h-screen p-12 flex items-center justify-center text-[#111827]">
        <div className="flex items-center gap-3 font-extrabold text-sm text-gray-500">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-extrabold text-[#111827] tracking-tight flex items-center gap-2">
              <Save className="h-6 w-6 text-[#F59E0B]" />
              Edit Team User
            </h1>
            <p className="text-xs font-semibold text-[#6B7280] mt-0.5">
              Update user details, role assignment, and permissions
            </p>
          </div>
        </div>
      </div>
