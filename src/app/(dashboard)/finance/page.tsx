  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-5">
        ...
        <Link
          href="/finance/new"
          className="px-4 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-extrabold transition flex items-center gap-2 shadow-md shadow-indigo-600/20 interactive-hover self-start sm:self-auto"
        >
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm shadow-slate-200/50 space-y-4">
        <form method="GET" className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-3">
          ...
          <div className="relative md:col-span-2">
            <Search className="absolute left-3.5 top-2.5 h-4 w-4 text-indigo-600" />
            <input
              type="text"
              name="q"
              defaultValue={query}
              placeholder="Search by invoice #, client name, remarks..."
              className="w-full bg-slate-50 border border-slate-300 rounded-xl pl-9 pr-3 py-2 text-xs text-slate-900 font-bold placeholder:text-slate-500 focus:outline-none focus:border-indigo-600 focus:ring-2 focus:ring-indigo-600/20 transition-all duration-200"
            />
          </div>
          ...
          <div className="relative">
            ...
            <select
              ...
              className="w-full bg-slate-50 border border-slate-300 rounded-xl pl-8 pr-3 py-2 text-xs text-slate-900 font-bold focus:outline-none focus:border-indigo-600 focus:ring-2 focus:ring-indigo-600/20 transition-all duration-200"
            >
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm shadow-slate-200/50 overflow-hidden">
        <div className="p-4 border-b border-slate-200 flex items-center justify-between">
          ...
          <span className="text-[11px] text-slate-600 font-bold">Page {page} of {totalPages || 1}</span>
        </div>
        ...
        {invoiceList.length > 0 ? (
              ...
              return (
                ...
                >
                  <div className="space-y-1">
                    ...
                    <div className="text-xs text-slate-600 font-semibold flex flex-wrap items-center gap-x-4 gap-y-1">
                      <span>
                  ...
                  <div className="flex items-center justify-between md:justify-end gap-6 text-xs">
                    ...
                    <Link
                      href={`/finance/${inv.id}`}
                      className="px-4 py-2 bg-slate-900 hover:bg-indigo-600 text-white font-extrabold rounded-xl text-xs shadow-sm hover:shadow-indigo-600/20 interactive-hover transition-all duration-200 flex items-center gap-1.5"
                    >
          ...
          <div className="py-12 text-center text-slate-600 text-xs font-bold space-y-2">
        ...
        {totalPages > 1 && (
          <div className="p-4 border-t border-slate-200 flex items-center justify-between text-xs font-bold">
            <Link
              ...
              className={`px-4 py-2 bg-white border border-slate-300 text-slate-800 rounded-xl font-extrabold transition-all duration-200 shadow-2xs ${
